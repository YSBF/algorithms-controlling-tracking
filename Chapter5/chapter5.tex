...
The validation of the estimation is the first test made before flying. The robot was turned on and moved around the room by hands with the motors unarmed for safety. The on board estimation of the attitude was copared with the vision estimate from the mocap. \\
...
\begin{figure}[H]
	\centering
	\noindent
	\includegraphics[width=1\textwidth]{roll_esti.png}
...
\begin{figure}[H]
	\centering
	\noindent
	\includegraphics[width=1\textwidth]{pitch_esti.png}
...
The yaw estimation is nearly perfect. It does not present the same delay as for Roll and Pitch mainly because it relies mostly on the mocap feedback. At 44 seconds a jump is present because is the point in which the angle abruptly changes from $\pi$ to $\-pi$. However this does not affect the robot dynamics.

\begin{figure}[H]
	\centering
	\noindent
	\includegraphics[width=1\textwidth]{yaw_esti.png}
...
\end{figure}
\noindent
Position estimation results are not present because they are optimal and the two curves are perfectly superimposed.

\subsection{Controller results}

The preliminary test to evaluate the controller performance wa to pass to the robot variable set points in a square wave fashion, in other words to move between the corners of a square. The plots for the position and the yaw are presented since they are the most relevant.

\paragraph{X and Y position} The performance over the horizontal plane 
