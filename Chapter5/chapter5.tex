...
\begin{equation}
 \boldsymbol{th_{sp}} = K_p^{vel}  \boldsymbol{e_v} + K_d^{vel} \dot{ \boldsymbol{e_v}} + K_i^{vel} \int  \boldsymbol{e_v}
\end{equation}
At this point we can calculate attitude set points, in the form of rotation matrix to avoid singularities, from thrust vector. The desired z axis of the robot is
\begin{equation}
...
\end{equation}
and the operator $\wedge$ is called vee map. 
