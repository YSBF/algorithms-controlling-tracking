...
The validation of the estimation is the first test made before flying. The robot was turned on and moved around the room by hands with the motors unarmed for safety. The on board estimation of the attitude was compared with the vision estimate from the mocap. \\

\noindent
\paragraph{Roll estimation} 

The estimation for the roll is very good. The vision signal is delayed respect to the on board estimation due to the radio link. At almost 15 seconds there is a spike due to the cameras losing track of the object.
...
As for roll, the estimation of the pitch il consistent. Similar spike appears at 15 seconds.

\begin{figure}[H]
	\centering
	\noindent
	\includegraphics[width=0.9\textwidth]{pitch_esti.png}
	\caption[Roll estimation]{Estimation for the pitch angle, in black the mocap value while in green the estimated value. Pitch is measured in radiants.}
...
The yaw estimation is nearly perfect. It does not 
 the same delay as for Roll and Pitch mainly because it relies mostly on the mocap feedback. At 44 seconds a jump is present because is the point in which the angle abruptly changes from $-\pi$ to $\pi$. However this does not affect the robot dynamics because internal computations are done with rotation matrix representation(see section \ref{sec:attcontrol}).

\begin{figure}[H]
	\centering
	\noindent
	\includegraphics[width=0.9\textwidth]{yaw_esti.png}
	\caption[Roll estimation]{Estimation for the yaw angle, in black the mocap value while in blue the estimated value. Yaw is measured in radiants.}
...
During the preliminary test,in order to evaluate the controller performances, we pass to the robot variable set points in a square wave fashion. The plots for the position and the yaw are presented since they are the most relevant.

\paragraph{X and Y position} The performance over the horizontal plane is decent. Oscillations are mainly induced by the wind generated by the rotors, bouncing on the walls and back to the robot.
...
\begin{figure}[H]
	...
	\label{figure:yconv}
\end{figure}
The maximum error is about 10 centimeter in hovering.\\
\paragraph{Z position} The z converges more slowly when the robot starts, this is because the integrator in the desired thrust vector need to be saturated. By putting as initial condition of the integrator the value of the gravity force one can avoid this problem which appears only at the beginning during flight. At 70 seconds the landing maneuver starts.
...
\paragraph{Yaw convergence} The yaw is commanded by sending yaw setpoints in a ramp fashion. The ramp appears segmented to the robot since the sending fequence is much lower than the processing rate on the board.

\begin{figure}[H]
	\centering
	\noindent
	\includegraphics[width=1\textwidth]{yaw_conv.png}
	\caption[Yaw tracking]{Yaw set point tracking, the blue is the real value while the black is the setpoint}
	\label{figure:yawconv}
\end{figure}

\noindent
The last experiment (figure \ref{figure:hover}) consists in letting the robot hover on a point and perturbing it by pushing and pulling with a stick. At each spike on x, y and z an external force is applied.

\begin{figure}[h]
	\centering
	\noindent
	\includegraphics[width=1\textwidth]{perturbation.png}
	\caption[Static hover with external perturbations]{Red plot is x, green is is y and blue is z. The horizontal setpoint is at (0 , 0) and the height is variable. Light blue represent the setpoint on z.}
	\label{figure:hover}
\end{figure}
